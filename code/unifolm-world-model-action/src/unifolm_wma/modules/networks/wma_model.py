...
class ResBlock(TimestepBlock):
    ...
    def __init__(self,
        ...
        # Inference can opt into NDHWC-compatible temporal convolutions while
        # keeping the default training layout unchanged.
        self.use_channels_last_3d = False
...
class WMAModel(nn.Module):
    ...
    def forward(self,
                ...
                action_observation_cond: Tensor | None = None,
                state_observation_cond: Tensor | None = None,
        ...
        if active_head not in ("both", "action", "state"):
            raise ValueError(f"Unsupported active head: {active_head}")

        if not self.base_model_gen_only:
            ba, _, _ = x_action.shape
            if active_head in ("both", "action"):
                a_y = self.action_unet(
                    x_action,
                    timesteps[:ba],
                    hs_a,
                    context_action[:2],
                    observation_global_cond=action_observation_cond,
                    **kwargs)
            else:
                # Metadata-compatible placeholder; the phase-aware sampler
                # never consumes this inactive stream.
                a_y = x_action
            # Predict state
            if active_head in ("both", "state"):
                state_timesteps = timesteps[:ba] if b > 1 else timesteps
                s_y = self.state_unet(
                    x_state,
                    state_timesteps,
                    hs_a,
                    context_action[:2],
                    observation_global_cond=state_observation_cond,
                    **kwargs)
            else:
                s_y = x_state
        else:
