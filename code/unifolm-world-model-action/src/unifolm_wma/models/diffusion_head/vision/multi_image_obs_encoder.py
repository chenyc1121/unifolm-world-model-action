...
class FixedChannelNormalize(nn.Module):
    """Compile-safe channel normalization for fixed image statistics."""

    def __init__(self, mean, std):
        super().__init__()
        self._mean_values = tuple(float(value) for value in mean)
        self._std_values = tuple(float(value) for value in std)
        self.register_buffer("mean",
                             torch.tensor(self._mean_values).view(1, -1, 1,
                                                                  1),
                             persistent=False)
        self.register_buffer("std",
                             torch.tensor(self._std_values).view(1, -1, 1, 1),
                             persistent=False)

    def materialize_runtime_buffers(self, device='cpu'):
        """Recreate non-persistent buffers after meta checkpoint loading."""
        self.mean = torch.tensor(self._mean_values,
                                 dtype=self.mean.dtype,
                                 device=device).view(1, -1, 1, 1)
        self.std = torch.tensor(self._std_values,
...
class MultiImageObsEncoder(ModuleAttrMixin):
        ...
        for key, attr in obs_shape_meta.items():
            ...
            if type == 'rgb':
                ...
                if not use_dinoSiglip:
                    ...
                    if imagenet_norm:
                        this_normalizer = FixedChannelNormalize(
    ...
    def output_shape(self, resnet_output_shape=False):
        ...
        if hasattr(self, 'spatial_softmax'):
            self.spatial_softmax.kps = None
        return output_shape
