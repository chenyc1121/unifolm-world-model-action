...
class ConditionalUnet1D(nn.Module):
    ...
    def encode_observation(self,
        ...
        """Encode the fixed image/state condition once per DDIM sampling run."""
        if not self.use_linear_act_proj:
            raise RuntimeError(
                "Cached observation encoding requires use_linear_act_proj.")

        observation = {
            'image': cond[0].permute(0, 2, 1, 3, 4),
            'agent_pos': cond[1],
        }
        observation['image'] = rearrange(
            observation['image'], 'b t c h w -> (b t) c h w')
        observation['agent_pos'] = rearrange(
            observation['agent_pos'], 'b t d -> (b t) d')
        global_cond = self.obs_encoder(observation)
        global_cond = rearrange(global_cond,
                                '(b t) d -> b 1 (t d)',
                                b=batch_size,
                                t=self.n_obs_steps)
        return repeat(global_cond,
                      'b c d -> b (repeat c) d',
                      repeat=horizon)
    ...
    def forward(self,
        ...
        if self.use_linear_act_proj:
            ...
            if observation_global_cond is None:
                global_cond = self.encode_observation(cond, B, T)
            else:
                global_cond = observation_global_cond
        else:
