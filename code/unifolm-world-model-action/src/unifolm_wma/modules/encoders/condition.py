...
def _materialize_open_clip_attention_mask(model, device):
    """Recreate OpenCLIP's non-persistent causal text-attention mask."""
    attention_mask = model.attn_mask
    mask = torch.empty(tuple(attention_mask.shape),
...
class FrozenOpenCLIPEmbedder(AbstractEncoder):
    ...
    def __init__(self,
        ...
        super().__init__()
        assert layer in self.LAYERS
        init_device = torch.empty(0).device
        model, _, _ = open_clip.create_model_and_transforms(
            arch, device=init_device, pretrained=version)
        del model.visual
        self.model = model
    ...
    def materialize_runtime_buffers(self, device='cpu'):
        """Recreate OpenCLIP buffers omitted from its state dict."""
        _materialize_open_clip_attention_mask(self.model, device)
...
class FrozenOpenCLIPImageEmbedder(AbstractEncoder):
    ...
    def materialize_runtime_buffers(self, device='cpu'):
        """Recreate buffers omitted from the OpenCLIP state dict."""
