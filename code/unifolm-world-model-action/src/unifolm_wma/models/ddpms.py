...
class DDPM(pl.LightningModule):
    ...
    def __init__(
        ...
        logvar_kwargs = {} if self.learn_logvar else {'device': 'cpu'}
    ...
    def register_schedule(self,
        ...
        if self.parameterization == "eps":
            ...
            alphas_cumprod_tensor = torch.Tensor(alphas_cumprod)
            lvlb_weights = 0.5 * np.sqrt(alphas_cumprod_tensor) / (
                2. - alphas_cumprod_tensor)
            lvlb_weights = lvlb_weights.to(self.betas.device)
        ...
        if self.lvlb_weights.device.type != 'meta':
            assert not torch.isnan(self.lvlb_weights).all()

    def materialize_runtime_buffers(self, device='cpu') -> None:
        """Recreate diffusion buffers omitted from the checkpoint state dict."""
        ...
        if self.parameterization == "eps":
            ...
            alphas_cumprod_cpu = alphas_cumprod.to(device='cpu')
            lvlb_weights = 0.5 * np.sqrt(alphas_cumprod_cpu) / (
                2. - alphas_cumprod_cpu)
            lvlb_weights = lvlb_weights.to(device=device)
        elif self.parameterization == "v":
            lvlb_weights = torch.ones_like(betas)
        else:
            raise NotImplementedError("mu not supported")
...
class LatentVisualDiffusion(LatentDiffusion):
    ...
    def _init_dp_noise_scheduler(self, config: OmegaConf) -> None:
        ...
        """
        # Diffusers schedulers are regular Python objects rather than modules,
        # so checkpoint assignment cannot materialize their internal tensors.
